...
package order_etl.sort_and_aggregate_0_1;
...
/**
 * Job: Sort_and_aggregate Purpose: Sorting and aggregating the input data<br>
 * Description: This scenario describes a Job that sorts the entries of the input data based on two columns and displays the sorted data on the console, then aggregates the sorted data based on one column and displays the aggregated data on the console. <br>
 * @author gstreet@copyright.com
 * @version 7.1.1.20181026_1147
 * @status 
 */
public class Sort_and_aggregate implements TalendJob {
	...
	private final String jobName = "Sort_and_aggregate";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Sort_and_aggregate.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Sort_and_aggregate.this, new Object[] { e,
	...
	public void tAggregateSortedRow_1_error(Exception exception,
			String errorComponent, final java.util.Map<String, Object> globalMap)
			throws TalendException {
	...
	public static class row4Struct implements
		...
		final static byte[] commonByteArrayLock_ORDER_ETL_Sort_and_aggregate = new byte[0];
		static byte[] commonByteArray_ORDER_ETL_Sort_and_aggregate = new byte[0];

		public String AggTeam;

		public String getAggTeam() {
			return this.AggTeam;
		}

		public Integer AggCount;

		public Integer getAggCount() {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_ORDER_ETL_Sort_and_aggregate.length) {
					if (length < 1024
							&& commonByteArray_ORDER_ETL_Sort_and_aggregate.length == 0) {
						commonByteArray_ORDER_ETL_Sort_and_aggregate = new byte[1024];
					} else {
						commonByteArray_ORDER_ETL_Sort_and_aggregate = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_ORDER_ETL_Sort_and_aggregate, 0,
						length);
				strReturn = new String(
						commonByteArray_ORDER_ETL_Sort_and_aggregate, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_ORDER_ETL_Sort_and_aggregate) {

				try {

					int length = 0;

					this.AggTeam = readString(dis);

					this.AggCount = readInteger(dis);

					this.MinAge = readInteger(dis);

					this.MaxAge = readInteger(dis);

					this.AvgAge = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.AggTeam, dos);

				// Integer

				writeInteger(this.AggCount, dos);

				// Integer

				writeInteger(this.MinAge, dos);

				// Integer

				writeInteger(this.MaxAge, dos);

				// Integer

				writeInteger(this.AvgAge, dos);
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("AggTeam=" + AggTeam);
			sb.append(",AggCount=" + String.valueOf(AggCount));
			sb.append(",MinAge=" + String.valueOf(MinAge));
			sb.append(",MaxAge=" + String.valueOf(MaxAge));
			sb.append(",AvgAge=" + String.valueOf(AvgAge));
			sb.append("]");
	...
	public void tFixedFlowInput_1Process(
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				class Comparablerow1Struct extends row1Struct implements
					...
					public int compareTo(Comparablerow1Struct other) {
						...
						} else if (this.Team != null && other.Team == null) {
							return 1;
				...
				result_tFixedFlowInput_1
						.append("MTtUaG9tYXM7Mjg7Q29tcG9uZW50IFRlYW0KMjtIYXJyeTszMjtEb2MgVGVhbQozO0pvaG47MjY7");
				result_tFixedFlowInput_1
						.append("Q29tcG9uZW50IFRlYW0KNDtOaWNvbGFzOzI3O1FBIFRlYW0KNTtHZW9yZ2U7MjQ7Q29tcG9uZW50");
				result_tFixedFlowInput_1
						.append("IFRlYW0KNjtQZXRlcjszMDtEb2MgVGVhbQo3O1RlZGR5OzIzO1FBIFRlYW0KODtKYW1lczsyNjtD");
				result_tFixedFlowInput_1.append("b21wb25lbnQgVGVhbQ==");
				...
				for (int i_tFixedFlowInput_1 = 0; i_tFixedFlowInput_1 < 1; i_tFixedFlowInput_1++) {
					...
					while (fid_tFixedFlowInput_1.nextRecord()) {
						...
						if (0 < fid_tFixedFlowInput_1
							...
							row1.Id = (colContent == null || colContent.trim()
									.length() == 0) ? null : ParserUtils
									.parseTo_Integer(colContent);
						} else {
						...
						if (1 < fid_tFixedFlowInput_1
							...
							row1.Name = (colContent == null || colContent
						...
						if (2 < fid_tFixedFlowInput_1
								.getColumnsCountOfCurrentRow()) {
							String colContent = fid_tFixedFlowInput_1.get(2);
							row1.Age = (colContent == null || colContent.trim()
									.length() == 0) ? null : ParserUtils
									.parseTo_Integer(colContent);
						} else {
						...
						if (3 < fid_tFixedFlowInput_1
								.getColumnsCountOfCurrentRow()) {
							String colContent = fid_tFixedFlowInput_1.get(3);
							row1.Team = (colContent == null || colContent
									.length() == 0) ? null : colContent;
						} else {
						...
						arrayRowtSortRow_1_SortOut.Age = row1.Age;
						arrayRowtSortRow_1_SortOut.Team = row1.Team;
				...
				util_tLogRow_2.addRow(new String[] { "AggTeam", "AggCount",
				...
				/**
				 * [tAggregateSortedRow_1 begin ] start
				 */

				ok_Hash.put("tAggregateSortedRow_1", false);
				start_Hash.put("tAggregateSortedRow_1",
						System.currentTimeMillis());

				currentComponent = "tAggregateSortedRow_1";
				...
				int tos_count_tAggregateSortedRow_1 = 0;

				final row4Struct[] emmitArray_tAggregateSortedRow_1 = new row4Struct[2];
				emmitArray_tAggregateSortedRow_1[0] = new row4Struct();
				emmitArray_tAggregateSortedRow_1[1] = new row4Struct();
				int nb_line_tAggregateSortedRow_1 = 0;
				int currentRowIndex_tAggregateSortedRow_1 = 0;
				boolean flag_tAggregateSortedRow_1 = true;// flag for the
															// encounter of
															// first row.
				String group_Team_tAggregateSortedRow_1 = null;
				int count_Name_false_tAggregateSortedRow_1 = 0;
				Integer min_Age_false_tAggregateSortedRow_1 = null;
				Integer max_Age_false_tAggregateSortedRow_1 = null;
				double sum_Age_false_tAggregateSortedRow_1 = 0d;
				int count_Age_false_tAggregateSortedRow_1 = 0;

				/**
				 * [tAggregateSortedRow_1 begin ] stop
				 */
				...
				util_tLogRow_1.addRow(new String[] { "Id", "Name", "Age",
						"Team", });
				...
				for (int i_tSortRow_1_SortIn = 0; i_tSortRow_1_SortIn < array_tSortRow_1_SortIn.length; i_tSortRow_1_SortIn++) {
					...
					row2.Team = current_tSortRow_1_SortIn.Team;
					...
					if (row2.Age != null) { //
						row_tLogRow_1[2] = String.valueOf(row2.Age);

					} //

					if (row2.Team != null) { //
						row_tLogRow_1[3] = String.valueOf(row2.Team);
					...
					/**
					 * [tAggregateSortedRow_1 main ] start
					 */
					...
					currentRowIndex_tAggregateSortedRow_1++;
					boolean sameGroup_tAggregateSortedRow_1 = true;
					if (flag_tAggregateSortedRow_1) {
						flag_tAggregateSortedRow_1 = false;
						group_Team_tAggregateSortedRow_1 = row3.Team;
						count_Name_false_tAggregateSortedRow_1 = 1;
						min_Age_false_tAggregateSortedRow_1 = row3.Age;
						max_Age_false_tAggregateSortedRow_1 = row3.Age;
						if (row3.Age != null) {
							sum_Age_false_tAggregateSortedRow_1 = (double) row3.Age;
						} else {
							sum_Age_false_tAggregateSortedRow_1 = (double) 0;
						}
						count_Age_false_tAggregateSortedRow_1 = 1;
					} else {
						while (true) {
							if (group_Team_tAggregateSortedRow_1 == null) {
								if (row3.Team != null) {
									sameGroup_tAggregateSortedRow_1 = false;
									break;
								}
							} else {
								if (group_Team_tAggregateSortedRow_1 == null
										|| !group_Team_tAggregateSortedRow_1
												.equals(row3.Team)) {
									sameGroup_tAggregateSortedRow_1 = false;
									break;
								}
						...
						if (sameGroup_tAggregateSortedRow_1) {
							count_Name_false_tAggregateSortedRow_1++;
							if (row3.Age != null) {
								if (min_Age_false_tAggregateSortedRow_1 == null
										|| min_Age_false_tAggregateSortedRow_1
												.compareTo(row3.Age) > 0) {
									min_Age_false_tAggregateSortedRow_1 = row3.Age;
								}
							}
							if (row3.Age != null) {
								if (max_Age_false_tAggregateSortedRow_1 == null
										|| max_Age_false_tAggregateSortedRow_1
												.compareTo(row3.Age) < 0) {
									max_Age_false_tAggregateSortedRow_1 = row3.Age;
								}
							}
							if (row3.Age != null) {
								sum_Age_false_tAggregateSortedRow_1 += row3.Age;
							}
							count_Age_false_tAggregateSortedRow_1++;
						}// if_same_group

					}

					int tempCount_tAggregateSortedRow_1 = -1;
					if (!sameGroup_tAggregateSortedRow_1) {
						tempCount_tAggregateSortedRow_1++;
						emmitArray_tAggregateSortedRow_1[tempCount_tAggregateSortedRow_1].AggTeam = group_Team_tAggregateSortedRow_1;
						emmitArray_tAggregateSortedRow_1[tempCount_tAggregateSortedRow_1].AggCount = (int) count_Name_false_tAggregateSortedRow_1;
						emmitArray_tAggregateSortedRow_1[tempCount_tAggregateSortedRow_1].MinAge = min_Age_false_tAggregateSortedRow_1;
						emmitArray_tAggregateSortedRow_1[tempCount_tAggregateSortedRow_1].MaxAge = max_Age_false_tAggregateSortedRow_1;
						double avg_3_tAggregateSortedRow_1 = 0d;
						if (count_Age_false_tAggregateSortedRow_1 > 0) {
							avg_3_tAggregateSortedRow_1 = sum_Age_false_tAggregateSortedRow_1
									/ count_Age_false_tAggregateSortedRow_1;
						}
						emmitArray_tAggregateSortedRow_1[tempCount_tAggregateSortedRow_1].AvgAge = (int) avg_3_tAggregateSortedRow_1;
						group_Team_tAggregateSortedRow_1 = row3.Team;
						count_Name_false_tAggregateSortedRow_1 = 1;
						min_Age_false_tAggregateSortedRow_1 = row3.Age;
						max_Age_false_tAggregateSortedRow_1 = row3.Age;
					...
					if (currentRowIndex_tAggregateSortedRow_1 == 8) {
						tempCount_tAggregateSortedRow_1++;
						emmitArray_tAggregateSortedRow_1[tempCount_tAggregateSortedRow_1].AggTeam = group_Team_tAggregateSortedRow_1;
						emmitArray_tAggregateSortedRow_1[tempCount_tAggregateSortedRow_1].AggCount = (int) count_Name_false_tAggregateSortedRow_1;
						emmitArray_tAggregateSortedRow_1[tempCount_tAggregateSortedRow_1].MinAge = min_Age_false_tAggregateSortedRow_1;
					...
					for (int i_tAggregateSortedRow_1 = 0; i_tAggregateSortedRow_1 <= tempCount_tAggregateSortedRow_1; i_tAggregateSortedRow_1++) {
						row4.AggTeam = emmitArray_tAggregateSortedRow_1[i_tAggregateSortedRow_1].AggTeam;
						row4.AggCount = emmitArray_tAggregateSortedRow_1[i_tAggregateSortedRow_1].AggCount;
						row4.MinAge = emmitArray_tAggregateSortedRow_1[i_tAggregateSortedRow_1].MinAge;
						row4.MaxAge = emmitArray_tAggregateSortedRow_1[i_tAggregateSortedRow_1].MaxAge;
						row4.AvgAge = emmitArray_tAggregateSortedRow_1[i_tAggregateSortedRow_1].AvgAge;
						nb_line_tAggregateSortedRow_1++;

						tos_count_tAggregateSortedRow_1++;

						/**
						 * [tAggregateSortedRow_1 main ] stop
						 */

						/**
						 * [tAggregateSortedRow_1 process_data_begin ] start
						 */

						currentComponent = "tAggregateSortedRow_1";

						/**
						 * [tAggregateSortedRow_1 process_data_begin ] stop
						 */
						...
						if (row4.AggTeam != null) { //
							row_tLogRow_2[0] = String.valueOf(row4.AggTeam);

						} //

						if (row4.AggCount != null) { //
							row_tLogRow_2[1] = String.valueOf(row4.AggCount);

						} //

						if (row4.MinAge != null) { //
							row_tLogRow_2[2] = String.valueOf(row4.MinAge);

						} //

						if (row4.MaxAge != null) { //
							row_tLogRow_2[3] = String.valueOf(row4.MaxAge);

						} //

						if (row4.AvgAge != null) { //
							row_tLogRow_2[4] = String.valueOf(row4.AvgAge);
					...
					/**
					 * [tAggregateSortedRow_1 process_data_end ] start
					 */

					currentComponent = "tAggregateSortedRow_1";

					/**
					 * [tAggregateSortedRow_1 process_data_end ] stop
					 */
				...
				/**
				 * [tAggregateSortedRow_1 end ] start
				 */

				currentComponent = "tAggregateSortedRow_1";

				globalMap.put("tAggregateSortedRow_1_NB_LINE",
						nb_line_tAggregateSortedRow_1);
				if (execStat) {
					if (resourceMap.get("inIterateVComp") == null
							|| !((Boolean) resourceMap.get("inIterateVComp"))) {
				...
				ok_Hash.put("tAggregateSortedRow_1", true);
				end_Hash.put("tAggregateSortedRow_1",
						System.currentTimeMillis());

				/**
				 * [tAggregateSortedRow_1 end ] stop
				 */
			...
			try {
				...
				/**
				 * [tAggregateSortedRow_1 finally ] start
				 */

				currentComponent = "tAggregateSortedRow_1";

				/**
				 * [tAggregateSortedRow_1 finally ] stop
				 */
	...
	public static void main(String[] args) {
		final Sort_and_aggregate Sort_and_aggregateClass = new Sort_and_aggregate();

		int exitCode = Sort_and_aggregateClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Sort_and_aggregate.class
					.getClassLoader().getResourceAsStream(
							"order_etl/sort_and_aggregate_0_1/contexts/"
									+ contextStr + ".properties");
			if (inContext == null) {
				inContext = Sort_and_aggregate.class
						.getClassLoader()
						.getResourceAsStream(
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : Sort_and_aggregate");
		}
...
/************************************************************************************************
 * 124112 characters generated by Talend Open Studio for Data Integration on the
 * September 27, 2019 3:53:03 PM EDT
 ************************************************************************************************/
