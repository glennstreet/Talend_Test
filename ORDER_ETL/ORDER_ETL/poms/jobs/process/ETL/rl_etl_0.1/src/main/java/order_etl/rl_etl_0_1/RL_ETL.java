 ...
 * @status 
 */
public class RL_ETL implements TalendJob {
	...
	private final String jobName = "RL_ETL";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(RL_ETL.this, new Object[] { e,
	...
	public static class row17Struct implements
		...
		final static byte[] commonByteArrayLock_ORDER_ETL_RL_ETL = new byte[0];
		static byte[] commonByteArray_ORDER_ETL_RL_ETL = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_ORDER_ETL_RL_ETL.length) {
					if (length < 1024
							&& commonByteArray_ORDER_ETL_RL_ETL.length == 0) {
						commonByteArray_ORDER_ETL_RL_ETL = new byte[1024];
					} else {
						commonByteArray_ORDER_ETL_RL_ETL = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_ORDER_ETL_RL_ETL, 0, length);
				strReturn = new String(commonByteArray_ORDER_ETL_RL_ETL, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_ORDER_ETL_RL_ETL) {
	...
	public static class row2Struct implements
		...
		public BigDecimal hard_copy_costs;

		public BigDecimal getHard_copy_costs() {
			return this.hard_copy_costs;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_ORDER_ETL_RL_ETL) {
				...
				try {
					...
					this.hard_copy_costs = (BigDecimal) dis.readObject();
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeObject(this.hard_copy_costs);
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String insert_tDBOutput_2 = "INSERT INTO \""
						...
						+ "\" (\"dwo_invoice_dimension_uid\",\"source_system_invoice_uid\",\"invoice_number\",\"invoice_source_system\",\"invoice_date\",\"invoice_status\",\"invoice_payment_date\",\"invoice_total_amount\",\"invoice_tax_amount\",\"invoice_discount_amount\",\"invoice_exchange_rate\",\"channel\",\"currency\",\"cart_type\",\"organization_name\",\"total_amount_in_transaction_currency\",\"original_amount_in_transaction_currency\",\"tax_amount_in_transaction_currency\",\"payment_status\",\"invoice_class\",\"organization_uid\",\"created_datetime\",\"created_by_user\",\"updated_datetime\",\"updated_by_user\",\"credit_memo_created_datetime\",\"credit_memo_updated_datetime\",\"invoice_hard_copy_costs\") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				String dbquery_tDBInput_2 = "SELECT fr.invoice_dimension_uid::INTEGER as dwo_invoice_dimension_uid,\n       fr.invoice_number as source_system_invoic"
						...
						+ "ted_datetime as credit_memo_updated_datetime,\n      fr.hard_copy_costs as invoice_hard_copy_costs\nFROM staging.dwo_rl_v_"
						+ "finance_report fr\nWHERE fr.payment_status != 'CANCELED'";
				...
				try {
					...
					while (rs_tDBInput_2.next()) {
						...
						if (colQtyInRs_tDBInput_2 < 28) {
							row2.hard_copy_costs = null;
						} else {

							if (rs_tDBInput_2.getObject(28) != null) {
								row2.hard_copy_costs = rs_tDBInput_2
										.getBigDecimal(28);
							} else {
								row2.hard_copy_costs = null;
							}
						...
						pstmt_tDBOutput_2.setBigDecimal(28,
								row2.hard_copy_costs);
	...
	public static class row6Struct implements
		...
		public BigDecimal hard_copy_amount;

		public BigDecimal getHard_copy_amount() {
			return this.hard_copy_amount;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_ORDER_ETL_RL_ETL) {
				...
				try {
					...
					this.hard_copy_amount = (BigDecimal) dis.readObject();
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeObject(this.hard_copy_amount);
		...
		public String toString() {
			...
			sb.append(",hard_copy_amount=" + String.valueOf(hard_copy_amount));
			sb.append("]");
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String dbquery_tDBInput_7 = "select nextval('staging.order_fact_seq') as dwo_order_fact_uid,\n       v.licensee_dimension_uid::NUMERIC as dwo_license"
						...
						+ ".total_amount\n       end as total_amount,\n       v.hard_copy_costs as hard_copy_amount\nfrom staging.dwo_rl_v_finance_rep"
						+ "ort v\nwhere v.invoice_created_datetime > '2017-06-30 23:59:59'\nand v.payment_status != 'CANCELED'\n-- and v.payment_statu"
						+ "s not like '%Credited%'\nand v.publisher != 'CCC Republication'\nand v.organization_name != 'TESTING'";
				...
				try {
					...
					while (rs_tDBInput_7.next()) {
						...
						if (colQtyInRs_tDBInput_7 < 17) {
							row6.hard_copy_amount = null;
						} else {

							if (rs_tDBInput_7.getObject(17) != null) {
								row6.hard_copy_amount = rs_tDBInput_7
										.getBigDecimal(17);
							} else {
								row6.hard_copy_amount = null;
							}
	...
	public static void main(String[] args) {
		final RL_ETL RL_ETLClass = new RL_ETL();

		int exitCode = RL_ETLClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = RL_ETL.class.getClassLoader()
					.getResourceAsStream(
							"order_etl/rl_etl_0_1/contexts/" + contextStr
									+ ".properties");
			if (inContext == null) {
				inContext = RL_ETL.class.getClassLoader().getResourceAsStream(
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : RL_ETL");
		}
...
/************************************************************************************************
 * 430800 characters generated by Talend Open Studio for Data Integration on the
 * December 19, 2019 7:04:12 PM EST
 ************************************************************************************************/
